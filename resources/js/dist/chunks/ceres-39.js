    ...
    getQualifiedVariations(attributeId, attributeValueId, unitId) {
      if (Object(_helper_utils__WEBPACK_IMPORTED_MODULE_1__["isDefined"])(attributeValueId)) {
        return this.variations.filter(variation => {
          return Object(_helper_utils__WEBPACK_IMPORTED_MODULE_1__["isDefined"])(variation.attributes.find(attribute => attribute.attributeId === attributeId && attribute.attributeValueId === attributeValueId));
        });
    ...
    filterVariations(attributes, unitId, strict, ignoreUnit) {
      ...
      const filteredVariations = this.variations.filter(variation => {
        ...
        for (const attributeId in attributes) {
          const variationAttribute = variation.attributes.find(variationAttribute => variationAttribute.attributeId === parseInt(attributeId)); // an attribute is not matching with selection
